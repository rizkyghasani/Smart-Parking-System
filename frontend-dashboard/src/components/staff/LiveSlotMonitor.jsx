...
import { parseWib } from '../../utils/time';
...
const LiveSlotMonitor = () => {
    ...
    const getLiveDuration = (entryTime) => {
        const diffMs = new Date() - (parseWib(entryTime) || new Date());
    ...
    return (
        <div className="bg-white p-6 rounded-2xl border border-[#E2E6EE] shadow-sm min-h-[500px] animate-in fade-in zoom-in-95 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h2 className="text-xl font-bold text-[#101828] flex items-center gap-2">
                        <Map className="text-[#26468A]" size={24} /> Live Slot Monitoring
                    </h2>
                    <p className="text-sm text-[#667085] mt-1">Pantau kendaraan aktif dan sinkronkan posisi fisik vs alokasi.</p>
                </div>
            </div>

            <div className="overflow-x-auto rounded-xl border border-[#E2E6EE] custom-scrollbar max-h-[500px] overflow-y-auto">
                <table className="w-full text-left text-sm">
                    ...
                    <tbody>
                        {loading ? (
                            <tr><td colSpan="4" className="py-8 text-center text-[#98A2B3]">Memuat data realtime...</td></tr>
                            ...
                            activeTransactions.map((tx) => {
                                ...
                                return (
                                    <tr key={tx.id} className={`border-b border-[#E2E6EE] hover:bg-[#F3F5F9]/70 transition-colors ${isViolation ? 'bg-rose-50/60' : ''}`}>
                                        <td className="py-4 px-5">
                                            <div className="flex flex-col gap-1.5">
                                                {/* Baris Alokasi Sistem */}
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] uppercase font-bold text-[#98A2B3] w-20">Alokasi:</span>
                                                    <span className="inline-flex items-center justify-center px-2.5 py-0.5 bg-[#26468A]/10 text-[#26468A] font-bold text-xs rounded-md">
                                                        Slot {allocatedCode}
                                                    </span>
                                                </div>
                                                ...
                                                {/* Baris Fisik / CCTV */}
                                                <div className="flex items-center gap-2">
                                                    <span className="text-[10px] uppercase font-bold text-[#98A2B3] w-20">Fisik:</span>
                                        ...
                                        <td className="py-4 px-5 font-black text-[#101828] tracking-widest text-base">
                                            {tx.plate_number}
                                        </td>
                                        ...
                                        <td className="py-4 px-5 text-right">
                                            <button 
                                                ...
                                                className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-bold transition-all ${isViolation ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-md shadow-rose-600/30' : 'bg-[#C97A1D]/10 hover:bg-[#C97A1D]/20 text-[#C97A1D]'}`}
                                            >
                            ...
                            <tr><td colSpan="4" className="py-12 text-center text-[#98A2B3] italic">Tidak ada kendaraan yang sedang parkir.</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-[#101828]/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-6 rounded-3xl w-full max-w-md border border-[#E2E6EE] shadow-2xl animate-in fade-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-5">
                            <h3 className="text-lg font-bold text-[#101828] flex items-center gap-2">
                                <ArrowRightLeft className="text-[#C97A1D]" size={20} /> Override Slot Manual
                            </h3>
                            <button onClick={() => setShowModal(false)} className="text-[#98A2B3] hover:text-[#101828] bg-[#F3F5F9] p-1.5 rounded-lg transition-colors cursor-pointer"><X size={20}/></button>
                        </div>
                        
                        <form onSubmit={handleOverrideSubmit} className="space-y-4">
                            <div className="bg-[#F3F5F9] p-4 rounded-2xl border border-[#E2E6EE] text-center">
                                <p className="text-[10px] text-[#98A2B3] uppercase tracking-widest font-bold mb-1">Target Kendaraan</p>
                                <p className="text-2xl font-black text-[#101828] tracking-widest">{overrideForm.plate}</p>
                                <p className="text-sm text-[#667085] mt-1">Alokasi Awal: <span className="text-rose-500 font-black">{overrideForm.old_slot_code}</span></p>
                            </div>

                            <div>
                                <label className="block text-xs text-[#667085] uppercase tracking-widest font-bold mb-2">Pindah Ke Slot Baru</label>
                                <select 
                                    className="w-full bg-white border border-[#E2E6EE] text-[#475467] p-3 rounded-xl focus:border-[#C97A1D] focus:ring-1 focus:ring-[#C97A1D] focus:outline-none"
                            ...
                            <div>
                                <label className="block text-xs text-[#667085] uppercase tracking-widest font-bold mb-2">Alasan Pemindahan</label>
                                <input 
                                    type="text" 
                                    className="w-full bg-white border border-[#E2E6EE] text-[#475467] p-3 rounded-xl focus:border-[#C97A1D] focus:ring-1 focus:ring-[#C97A1D] focus:outline-none"
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowModal(false)} className="flex-1 bg-[#EEF1F5] hover:bg-[#E2E6EE] text-[#475467] py-3 rounded-xl font-bold transition-colors cursor-pointer">Batal</button>
                                <button type="submit" disabled={isSubmitting} className="flex-1 bg-[#C97A1D] hover:bg-[#b06a17] text-white py-3 rounded-xl font-bold transition-colors disabled:opacity-50 shadow-lg shadow-[#C97A1D]/30 cursor-pointer">
